							...
							<div class="table-wrapper">
								<table>
									...
									<tbody>
										...
										<?php 
											$select_query1 = "SELECT Nick_Name,Category,Chart_id, Audio_file_address, Audio_contents_title FROM Total_Audio_Contents ORDER BY View_count DESC"; 
											...
											while ($row1 = mysqli_fetch_array($result_set1)) {
												$audio_array[$audio_count] = (string)$audio_count;
												$img_array[$img_count] = (string)$img_count;

												$to_user_id = $row1['Nick_Name'];
    											$to_chart_id = $row1['Chart_id'];
												$to_category = $row1['Category'];
												
												$sql2 = "SELECT participants FROM User_Voice_Point WHERE ID='$to_user_id' AND Chart_id='$to_chart_id' AND Category='$to_category'";
    											$result2 = mysqli_query($link,$sql2);
												$user_voice_point_row = mysqli_fetch_array($result2); 
												
										?>

										<tr>
											<td>
												<?php echo $img_array[$img_count]+1;?>
											</td>
											<td>
												<?php printf($row1["Category"]);?>
											</td>
											<td style="text-align: left;">
												<a href="grade.php?<?php printf($row1["Category"]); ?>?<?php printf($row1["Chart_id"]); ?>?<?php printf($row1["Nick_Name"]);?>" style="text-decoration:none;">
													<strong><?php printf($row1["Audio_contents_title"]);?></strong>
												</a>
											</td>
											<td>
												<span><?php printf($row1["Nick_Name"]);?></span>
											</td>
											<td>
												<span class="grade">☺&nbsp;<?php echo $user_voice_point_row['participants']; ?></span>
											</td>
											<td>
												<audio id="a<?php echo $audio_array[$audio_count];?>">
													<source src="<?php echo $row1['Audio_file_address'];?>" type="audio/mpeg" />
												</audio>
												<img id="<?php echo $img_array[$img_count];?>" onclick="audio_play_pause(this.id)" style="width:50px;" src="../images/play6.png" value="<?php echo $img_array[$img_count]; ?>">
												<audio  id="a<?php echo $audio_array[$audio_count]; ?>">
													<source src="<?php echo $row1['Audio_file_address']; ?>" type="audio/mpeg" />
												</audio>
												<img id="<?php echo $img_array[$img_count];?>" onclick="stop(this.id)" style="width:50px;" src="../images/stop6.png" value="<?php echo $img_array[$img_count]; ?>">
											</td>
